...
loopback.modelBuilder._define = loopback.modelBuilder.define;
loopback.modelBuilder.define = (className, properties, settings, parent) => {
  if (!settings.unresolved && Object.keys(properties).length === 0) {
    settings.forceId = false;
    settings.idInjection = false;
  }
  return loopback.modelBuilder._define(className, properties, settings, parent);
}

function mixinMigration(PostgreSQL) {
  PostgreSQL.prototype._getPropertyDefinition = PostgreSQL.prototype.getPropertyDefinition;
  PostgreSQL.prototype.getPropertyDefinition = function (modelName, propName) {
    return this._getPropertyDefinition(modelName, propName || '');
  }
      ...
      get: () => {
        ...
        for (let propName of Object.keys(m.properties || {})) {
          Object.values(self._models).some(function (definition) {
            ...
            return (Object.values(aModel.relations || {}).some(function (rel) {
              const foreignRelation = rel.keyTo;
              if (!rel.polymorphic && rel.type === 'hasOne' &&
                rel.modelTo.modelName === modelName &&
                foreignRelation === propName) {
                addForeignKey(foreignRelation,
                  rel.modelFrom.modelName, rel.keyFrom);
                return true;
              }
              return false;
            }));
        ...
        for (let rel of Object.values(model.relations || {})) {
          if (!rel.polymorphic && rel.type === 'belongsTo') {
            addForeignKey(rel.keyFrom,
              rel.modelTo.modelName, rel.keyTo);
          }
        }
        return _foreignKeys;
      }
      ...
      get: () => {
        ...
        for (const rel of Object.values(m.model.relations)) {
          if (rel.type === 'belongsTo') {
            const polymorphic = rel.polymorphic;
            if (polymorphic) {
              ...
              if (rel.modelTo.name !== userModel && rel.modelTo.name != modelName) {
                fkRefKeys.push(rel.keyFrom);
              }
            }
          }
        }
        if (fkRefKeys.length == 2) {
          // an intervening model of many-to-many
          let id = 1;
          fkRefKeys.sort();
          const ids = [];
          fkRefKeys = fkRefKeys.map(propName => {
              const prop = m.properties[propName];
              prop.required = true;
              const idInfo = { name: propName, id: id++, property: prop };
              prop.index = idInfo.id;
              ids.push(idInfo);
              return self.column(modelName, propName);
          });
          m._ids = ids;
          const joinedColumnNames = fkRefKeys.join('_');
          const indexName = self.table(modelName) + '_' + joinedColumnNames + '_key';
          const indKeys = fkRefKeys.reduce((key, columName, index) => {
            key.push([columName, 'ASC']);
            return key;
          }, []);
          if (!Object.entries(_indexes).some(([anIndexName, anIndex]) => {
            return anIndexName.toLowerCase() === indexName && anIndex.keys.length === 2 &&
              anIndex.keys[0][0] === indKeys[0][0] && anIndex.keys[0][1] === indKeys[0][1] &&
              anIndex.keys[1][0] === indKeys[1][0] && anIndex.keys[1][1] === indKeys[1][1];
          })) {
  ...
  PostgreSQL.prototype.modifyNullabilityInActual = function(model, propName) {
    const self = this;
    const prop = self._models[model].properties[propName];
  ...
  PostgreSQL.prototype.addIndexes = function(model, actualIndexes, cb) {
    ...
    propNames.forEach(function(propName) {
      ...
      if (!found) {
        ...
        if (!kind && !type && typeof i === 'object' && i.unique === true) {
          kind = 'UNIQUE';
        }
    ...
    indexNames.forEach(function(indexName) {
      ...
      if (!found) {
        ...
        if (!kind && !type && typeof i === 'object' && i.options.unique === true) {
          kind = 'UNIQUE';
        }

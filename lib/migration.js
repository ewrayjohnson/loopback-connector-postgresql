  ...
  PostgreSQL.prototype.define = function (m) {
    ...
    Object.defineProperty(settings, 'indexes', {
      get: () => {
        ...
        if (fkRefKeys.length == 2 && m.model.base.modelName == 'Model') {
    ...
    indexNames.forEach(function(indexName) {
      ...
      if (!found) {
        ...
        const columns = i.keys.map(function(key) {
          let col = self.escapeName(self.column(model, key[0]));
          if (i.options && i.options.nullable && 
            i.options.nullable.includes(key[0])) {
            col = '(' + col + ' IS NULL)';
          }
          return col + (key[1] ? ' ' + key[1] : '');
        }).join(', ');
        ...
        if (!kind && !type && i.options.unique === true) {
          kind = 'UNIQUE';
        }

...
const { m } = require('strong-globalize/lib/globalize');
...
PostgreSQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  // special operators avoid praram placeholders
  sql = sql.replace(/\v/g, '?');
...
function arrayOper(columnName, expression) {
  expression = JSON.stringify(expression);
  columnName = columnName.replace('->>', '->');
  const sql = `CASE WHEN jsonb_typeof(${columnName})='array' ` +
    `THEN ${columnName}\v|? ELSE ${columnName}<@? END`;
  return {
    sql: sql,
    params: [JSON.parse(expression), expression]
  };
}
...
PostgreSQL.prototype._buildWhere = function(model, where) {
  ...
  for (const key in where) {
    ...
    if (expression === null || expression === undefined) {
    ...
    } else if (p.type == Boolean && expression && Object.keys(expression)[0] == 'neq') {
      expression = expression['neq'];
      stmt.merge(columnName + ' IS NOT ' + expression);
      ...
      if (operator === 'inq' && p.type.name == 'Object' && Array.isArray(expression)) {
        sqlExp = arrayOper(columnName, expression);
      } else {
      ...
      if (columnValue === null) {
        ...
        if (columnValue instanceof ParameterizedSQL) {
          ...
          if (p.type.name == 'Object' && Array.isArray(columnValue)) {
            sqlExp = arrayOper(columnName, columnValue);
          } else {
            sqlExp = {
              sql: columnName + '=?',
              params: [columnValue],
